...
int8_t FireboltCore::fuzz() {
  ...
  // Parquet Fuzzer
    ...
    auto status = parquet_fuzzer.Fuzz(this->queries, this->db_url, this->input_corpus,
  ...
  // Iceberg Fuzzer
    ...
    // For Iceberg fuzzing, we start the loop here as there is sequential fuzzing logic involved
    while (1) {
      auto status =
          iceberg_fuzzer.fuzz_metadata_random(this->queries, this->db_url, this->radamsa_output,
                                              this->execs, this->curl, this->metadata_corpus);
      if (status == -1) {
        return -1;
      }
      status = iceberg_fuzzer.fuzz_metadata_structured(
          this->queries, this->db_url, this->radamsa_output, this->execs, this->curl);
      if (status == -1) {
        return -1;
      }
      status = iceberg_fuzzer.fuzz_manifest_list_structured(
          this->queries, this->db_url, this->metadata_corpus, this->radamsa_output, this->execs,
          this->curl);
      if (status == -1) {
        return -1;
      }
