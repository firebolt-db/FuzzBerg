...
CSVFuzzer::CSVFuzzer(pid_t target_pid, std::string fuzzer_mutation_path) {
  ...
  mutated_file_ptr = std::fopen((fuzzer_mutation_path + "/fuzz.csv").c_str(), "wb");
...
int8_t CSVFuzzer::Fuzz(std::vector<std::string>& queries, std::string& db_url,
  ...
  while (1) {
    ...
    auto output_size =
        radamsa(reinterpret_cast<uint8_t*>(input_corpus[rand_].corpus), input_corpus[rand_].size,
                reinterpret_cast<uint8_t*>(radamsa_buffer), RADAMSA_BUFFER_SIZE, seed_generator());
