...
volatile sig_atomic_t interrupted = 0;  // flag to indicate if the process was interrupted
...
void interrupt(int signal) {
  ...
  if (target_pid > 0) {
    ...
    std::cout << "\033[1;33m\n[INFO] Sending SIGUSR1 to target (PID: " << target_pid
    ...
    std::cout << "\033[1;31m\n[INFO] Terminating target process (PID: " << target_pid
              << ")\033[0m\n";
...
int main(int argc, char* argv[]) {
  if (argc <= 1) {
    fprintf(stderr, "Please provide all the necessary arguments to fuzz target DB\n\n");
    exit(1);
  }
  ...
  std::string database;  // database name to fuzz (initializes the corresponding target class)
  std::string queries;   // path to JSON file containing queries to execute

  std::unique_ptr<fuzzberg::DatabaseHandler> fuzz_target;

  static struct option long_options[] = {{"database", required_argument, NULL, 'd'},
  ...
  // Parse all options in a single pass and initialize fuzz_target when database is found
  optind = 1;
  while ((result = getopt_long(argc, argv, "b:i:o:m:t:f:u:q:B:d:", long_options, &option_index)) !=
         -1) {
    switch (result) {
      case 'd':
        if (optarg) {
      ...
      default: /* '?' */
      {
        fprintf(stderr,
                ...
                "  -B, --bucket BUCKET_NAME    S3 bucket name for Iceberg (required if "
                "--format=iceberg)\n",
                argv[0]);
        exit(1);
      }
  ...
  arg_strings.insert(arg_strings.end(), user_bin_args.begin(), user_bin_args.end());
  ...
  if (format == "csv") {
    ...
    if (s3_bucket.empty()) {
      std::cerr << "Error: --bucket (-B) must be provided when using --format=iceberg\n";
      exit(1);
    }
  ...
  fuzz_target->fuzzer_mutation_path = fuzzer_mutation_path;  // store mutation_file_path in fuzzer
  ...
  if (queries_json.is_discarded() || !queries_json.contains("queries") ||
    ...
    std::cerr << "Error: Invalid JSON format in query file. Expected an array of queries under "
                 "'queries' key.\n";
  ...
  std::cout << "\nLoaded " << fuzz_target->queries.size() << " queries from " << queries
            << std::endl;
...
interrupt:                      // section executed on receiving SIGINT
  ...
  std::cout << "\n"
            << Yellow << std::left << std::setw(15) << "Executions:" << Reset << std::right << Green
            << std::setw(8) << _execs << Reset << "\n"
            << Yellow << std::left << std::setw(15) << "Elapsed Time:" << Reset << std::right
            << Green << std::setw(2) << days << "d " << std::setw(2) << hours << "h "
            << std::setw(2) << minutes << "m " << std::setw(2) << seconds << "s" << Reset << "\n\n";
